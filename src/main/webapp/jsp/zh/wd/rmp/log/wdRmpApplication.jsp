		...
		<tr>
			...
			<div id="mainGraph2" style="height:270px;"></div>
				<%-- <div class="user_manage">
					...
					<img src="${base}/static/rmp/images/healthy.png"
	...
	<form id="form1">
		<div class="user_manage">
			...
			<div class="hj_username">
				...
				<input name="serverStatus" type="hidden" class="hj_usernameinp" value="1">
					<span class="hj_usertxtname">服务编号：</span> <span><input
						...
						name="serverName" type="text" class="hj_usernameinp"> 
						</span> <span>&nbsp;&nbsp;
...
<script type="text/javascript">
	...
	function selectchild(pid,pname) {
		location.href = basePath + "/appAnalysis/appMonitor?pid=" + pid+"&pname="+pname;
	}
	
	//查看告警信息
	function selectAlertChild(pid) {
		location.href = basePath + "/appAnalysis/alertPage?pid=" + pid;
	}

	function update(id) {
		location.href = basePath
				+ "/jsp/zh/wd/ssp/manage/wdSspServerUpdate.jsp?id=" + id;
	}

	function select(id) {
		location.href = basePath
				+ "/jsp/zh/wd/ssp/manage/wdSspServerDetail.jsp?id=" + id;
	}

	function add() {
		location.href = basePath + "/jsp/zh/wd/ssp/manage/wdSspServerAdd.jsp";
	}

	function del(id) {
		$.ajax({
			type : "get",
			url : basePath + "/wdSspServer/delete",
			data : {
				"id" : id
			},
			async : true,
	...
	function alertDetails(){
		location.href = basePath + "/appAnalysis/alertPage";
/* 		if(parseInt($("#alertSum").text())>0){
	...
	var shadowColorRed = zrColor.getLinearGradient(
            0, 0, 200, 0,
            [[0, 'rgba(255, 55, 55,0.5)'],[1, 'rgba(255, 0, 0, 0.9)']]
        );
	...
	function graphQuery() {
		...
		$.ajax({
			type : "post",
			url : basePath + "/appAnalysis/recGraph",
			data : [],
			async : false,
			dataType : "json", //text
			beforeSend : function() {
		...
		$.ajax({
			type : "post",
			url : basePath + "/appAnalysis/histGraph",
			data : [],
			async : false,
			dataType : "json", //text
			beforeSend : function() {
	...
	function graphQuery2(){
		...
		$.ajax({
			type : "post",
			url : basePath + "/appAnalysis/healthyRate",
			data : [],
			async : false,
			dataType : "json", //text
			beforeSend : function() {
				...
				$.each(data.obj, function(i, map) {
					for ( var key in map) {
						...
						if(option2.series[0].data[0].value>=80){
							$("#status_label").html("应用服务状态很好，请继续保持！");
		...
		if(parseInt($("#alertSum").text())>0){ //如果出现告警情况，高亮显示所在服务
			$.ajax({
				type : "post",
				url : basePath + "/appAnalysis/alertDetail",
				data : [],
				async : false,
				dataType : "json", //text
				beforeSend : function() {
