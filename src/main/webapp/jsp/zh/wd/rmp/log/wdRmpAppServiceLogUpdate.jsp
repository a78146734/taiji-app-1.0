...
<script type="text/javascript" src="js/wdRmpAppServiceLogUpdate.js"></script>
<link rel="stylesheet" type="text/css" href="css/wdRmpAppServiceLogUpdate.css">
...
<script type="text/javascript">
		...
		function doQuery(){
			$.ajax({
		 		type: "get",
		 		url: "<%=basePath%>/wdRmpAppServiceLog/selectByPrimaryKey",
		 		data:{
		 			"id":"<%=id%>"
		 		},
		 		async: true,
			...
			sb.append("<tr><td class='tablecolor'>日志级别：").append("</td><td  ><input type='text' name='logLevel'  value='").append(das.obj.logLevel).append("' ></td>");
						...
						sb.append("<td  class='tablecolor'>服务标示：</td><td ><input type='text' name='logService'  value='").append(das.obj.logService).append("' ></td></tr>");
					 						 					 						 				    
		 				
			sb.append("<tr><td class='tablecolor'>服务路径：").append("</td><td  ><input type='text' name='logServicePath'  value='").append(das.obj.logServicePath).append("' ></td>");
			
					 						 					 						 				    
		 				
						sb.append("<td  class='tablecolor'>服务名称：</td><td ><input type='text' name='logServiceName'  value='").append(das.obj.logServiceName).append("' ></td></tr>");
			...
			sb.append("<tr><td class='tablecolor'>系统标示：").append("</td><td  ><input type='text' name='sysCode'  value='").append(das.obj.sysCode).append("' ></td>");
						...
						sb.append("<td  class='tablecolor'>调用结果状态（0失败,1成功）：</td><td ><input type='text' name='serviceStatus'  value='").append(das.obj.serviceStatus).append("' ></td></tr>");
					 						 					 						 				    
		 				
			sb.append("<tr><td class='tablecolor'>响应时间(单位：毫秒)：").append("</td><td  ><input type='text' name='serviceRuntime'  value='").append(das.obj.serviceRuntime).append("' ></td>");
		...
		function Submit() {
			...
			$.ajax({
		 		type: "post",
		 		url: "<%=basePath%>/wdRmpAppServiceLog/update",
		 		data:params,
		 		async: true,
		 		dataType: "text",
		 		beforeSend: function(){},  
