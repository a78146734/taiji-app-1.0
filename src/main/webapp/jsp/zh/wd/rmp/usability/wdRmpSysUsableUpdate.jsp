...
<script type="text/javascript" src="js/wdRmpSysUsableUpdate.js"></script>
<link rel="stylesheet" type="text/css" href="css/wdRmpSysUsableUpdate.css">
<style type="text/css">
	body{
	...
	<form id="form1" >
		<div class="user_manage">
			<p class="ri_jczctitle">
	           <span class="ri_jczctitletxt">编辑页面</span>
			...
			<div class="user_list">
	           	<table class="gridtable04 ">
	           		<tr>
						...
						<input type="text" id="sysName" name="sysName" placeholder="请输入系统名称" datatype="s1-20" errormsg="请输入1-20位字符">
						</td>
					...
					<tr>
						...
						<td><input id="sys_phone" type="text" name="sysPhone" placeholder="请输入系统IP" datatype="m" errormsg="请输入手机号码"></td>
					</tr>
	           	</table>
           	</div>
		</div>
...
<script type="text/javascript">
		...
		function doQuery(){
			$.ajax({
		 		...
		 		success: function(data){
		 			...
		 			$("#id").val(das.obj.id);
		 			$("#sysName").val(das.obj.sysName);
		 			$("#usableType").val(das.obj.usableType);
		 			$("#sys_url").val(das.obj.sysUrl);
		 			$("#sys_ip").val(das.obj.sysIp);
		 			$("#sys_port").val(das.obj.sysPort);
		 			$("#sys_phone").val(das.obj.sysPhone);
		...
		function Submit() {
			...
			console.log(params);
			validAjaxFormList(basePath+"/wdRmpSysUsable/update");
			$("#form1").submit();
			<%-- $.ajax({
		 		type: "post",
		 		url: "<%=basePath%>/wdRmpSysUsable/update",
		 		data:params,
		 		async: true,
		 		dataType: "text",
		 		beforeSend: function(){},  
